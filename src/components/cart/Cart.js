...
const Cart = ({ history }) => {
    ...
    const updateQuantity = (productId, newQuantity, size, crust) => {
        ...
        const updatedCart = cartItems.map(item => {
            if (item.product === productId) {
                return { 
                    ...
                    price: calculatePrice(item.price, newQuantity, size, crust) 
                };
            }
            return item;
        });
        ...
        updatedCart.forEach(item => {
            dispatch(addItemToCart(item.product, item.quantity, size, crust));
        });
    };

    const calculatePrice = (basePrice, quantity, size, crust) => {
        ...
        return newPrice * quantity; // Tính toán lại giá theo số lượng
    };

    const increaseQty = (id, quantity, stock, size, crust) => {
        const newQty = quantity + 1;
        if (newQty > stock) return;
        updateQuantity(id, newQty, size, crust);
    };

    const decreaseQty = (id, quantity, size, crust) => {
        const newQty = quantity - 1;
        if (newQty <= 0) return;
        updateQuantity(id, newQty, size, crust);
    };
                                    ...
                                    <div className="cart-item">
                                        <div className="row">
                                            ...
                                            <div className="col-5 col-lg-3">
                                                ...
                                                <p>Loại đế: <strong>{item.crust}</strong></p>
                                            </div>
                                            ...
                                            <div className="col-4 col-lg-3 mt-4 mt-lg-0">
                                                <div className="stockCounter d-inline">
                                                    <span className="btn btn-danger minus" onClick={() => decreaseQty(item.product, item.quantity, item.size, item.crust)}>-</span>
                                                    <input type="number" className="form-control count d-inline" value={item.quantity} readOnly />
                                                    <span className="btn btn-primary plus" onClick={() => increaseQty(item.product, item.quantity, item.stock, item.size, item.crust)}>+</span>
                                                </div>
                                            </div>
